...
const DataFilterComponent = ({ data }) => {
  ...
  const handleFilter = () => {
    ...
    setFilteredChartData([
      ['Date', 'A', 'B', 'C'],
      ...filteredData.map(item => [item.Date, item.Branch === 'A' ? item.Total : 0, item.Branch === 'B' ? item.Total : 0, item.Branch === 'C' ? item.Total : 0]),
      ['Total', Math.min(branchTotals.A, 10000), Math.min(branchTotals.B, 10000), Math.min(branchTotals.C, 10000)],
    ]);
  };
